...
namespace srd5 {
    ...
    public class CombattantTest {
        ...
        public void TakeDamageTest() {
            ...
            ogre.TakeDamage(new DamageSource(DamageSourceType.OTHER, this, ogre), cold.Type, cold.Dice.RollCritical());
            Assert.InRange<int>(ogre.HitPoints, hp - 34, hp - 14);
            hp = ogre.HitPoints;
            Damage acid = new Damage(DamageType.ACID, "1d4+3");
            ogre.TakeDamage(new DamageSource(DamageSourceType.OTHER, this, ogre), acid.Type, acid.Dice.Roll());
            Assert.Equal(hp, ogre.HitPoints);
            Damage lightning = new Damage(DamageType.LIGHTNING, "1d12");
            ogre.TakeDamage(new DamageSource(DamageSourceType.OTHER, this, ogre), lightning.Type, lightning.Dice.Roll());
            Assert.InRange<int>(ogre.HitPoints, hp - 6, hp);
            Assert.Throws<Srd5ArgumentException>(delegate {
                ogre.TakeDamage(new DamageSource(DamageSourceType.OTHER, this, ogre), DamageType.ACID, -100);
            });
            Assert.Throws<Srd5ArgumentException>(delegate {
                ogre.HealDamage(-100);
            });
            ogre.TakeDamage(new DamageSource(DamageSourceType.OTHER, this, ogre), DamageType.PIERCING, ogre.HitPoints);
